...
void RemoteThread::run() {
	...
	int counter = 0;
	while (!isInterrupted()) {
		gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID, GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_HORIZONTAL_ABSOLUT,0)));
		usleep(3000000);
gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID, GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_VERTICAL_ABSOLUT,90)));
		usleep(3000000);
gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID, GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_VERTICAL_ABSOLUT,0)));
		usleep(3000000);
gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID, GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_HORIZONTAL_ABSOLUT,90)));
		usleep(3000000);
gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID, GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_VERTICAL_ABSOLUT,90)));
		usleep(3000000);
gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID, GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_VERTICAL_ABSOLUT,0)));
		usleep(3000000);
gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID, GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_HORIZONTAL_ABSOLUT,180)));
		usleep(3000000);
gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID, GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_VERTICAL_ABSOLUT,90)));
		usleep(3000000);
gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID, GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_VERTICAL_ABSOLUT,0)));
...
//			gripperQueue->enqueue(new Packet(MOTOR_QUEUE_ID,GRIPPER_QUEUE_ID,new GripperMessage(GRIPPER_MOVE_HORIZONTAL_ABSOLUT,degree)));
//
//		}
