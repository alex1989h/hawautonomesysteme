...
uint16_t RemoteControl::channelValues[max_channels];
...
void RemoteControl::updateChannels(){
  ...
  for(uint8_t pin = start_pin; pin < joystick_channels+start_pin; pin ++){
    channelValues[pin-start_pin] = pulseIn(pin, HIGH);
  }
  // Send all channel_values to Slave;
  // Read buttons
  channelValues[button_pin-start_pin] = pulseIn(button_pin,HIGH);
  ...
  switch(button_pin){
    case start_pin+4:
      stopButton = getButtonState(channelValues[button_pin-start_pin]);
      break;
    case start_pin+5:
      autonomousButton = getButtonState(channelValues[button_pin-start_pin]);
      break;
    default:
      ;// do nothing
  }
